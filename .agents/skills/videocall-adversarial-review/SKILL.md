...
8. Do not create, push as review-ready, or request review for a PR while required tests are absent, applicable validation is red, behavioral claims are unverified, comment volume is over budget, or a blocker remains. A user may explicitly request a WIP push, but the unresolved status must be stated.
...
6. Apply the full rubric, including test obligations, mutation sensitivity, execution paths, lifecycle, both transports, network conditions, scale, security, claim accuracy, and claim volume.
...
8. Post findings and a formal verdict. Missing required tests, merge conflicts, unexplained red CI, code blockers, or a comment-volume breach prohibit approval. Use `COMMENT` for self-authored PRs when GitHub prohibits a formal verdict, but apply the substantive labels.
