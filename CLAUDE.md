...
- **Audio publishes ONE layer** (`audio_published_layer_count()` returns `1`, #2279). Every audio packet is stamped layer 0, so the receiver's exact-match guard always matches and the relay never layer-filters audio (its filter is gated on `simulcast_layer_id != 0`). Audio quality now walks by **AQ tier** (`videocall_adaptive_audio_tier`), not by layer selection. `AUDIO_SIMULCAST_LAYER_KBPS` is the dormant publish-side ladder (its UI control and telemetry were removed by #2620/#2621) — **never explain an audio symptom with an audio layer mismatch.** The RECEIVE ladder is still 3 deep (`max_layers_for_kind(Audio)`, #2735), independent of publish depth, and `tick_audio_layer_chooser` is receive-side and DOES run every monitor tick — but `highest_available` only ever returns 0, so the `if audio < ah` gate is `0 < 0` and it never advertises a preference.
...
- **`videocall_audio_concealment_pct` overstates ~2x and its denominator moves.** The numerator counts NetEq ops (one per 10ms output frame, 100/s at full concealment); the denominator counts 20ms Opus packets (50/s), and the result is clamped to 100. So sustained concealment *while packets still arrive* reads 100%, and at a healthy 50 pkt/s denominator a reported p95 of 100% means "at least half the window" — under heavy loss the denominator falls and that floor falls with it. It is also a RATIO whose denominator ramps 0->50 pkt/s when a speaker starts transmitting, which fakes a spike at speech onset. Ratios between two populations survive both biases; **absolute magnitudes do not — corroborate with `videocall_neteq_expand_ops_per_sec` before quoting one.** A silent gap reads 0%, not 100% (gated on `packets_per_sec >= 2.0`).
