...
- **Audio publishes ONE layer** (`audio_published_layer_count()` returns `1`, #2279). Every audio packet is stamped layer 0, so the receiver's exact-match guard always matches and the relay never layer-filters audio (its filter is gated on `simulcast_layer_id != 0`). Audio quality now walks by **AQ tier** (`videocall_adaptive_audio_tier`), not by layer selection. `AUDIO_SIMULCAST_LAYER_KBPS` is the dormant publish-side ladder, pending #2620/#2621 — **never explain an audio symptom with an audio layer mismatch.** The RECEIVE ladder is still 3 deep (`max_layers_for_kind(Audio)`), independent of publish depth, and `tick_audio_layer_chooser` is receive-side and DOES run every monitor tick — but `highest_available` only ever returns 0, so the `if audio < ah` gate is `0 < 0` and it never advertises a preference.
