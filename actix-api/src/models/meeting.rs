...
impl Meeting {
    ...
    pub fn create(room_id: &str, started_at: DateTime<Utc>) -> Result<Self, Box<dyn Error + Send + Sync>> {
        let mut conn = get_connection_query()?;
        let row = conn.query_one(
            "
            INSERT INTO meetings (room_id, started_at, ended_at) 
            VALUES ($1, $2, NULL) 
   ...
   /// End a meeting by setting ended_at timestamp
    pub async fn end_meeting(room_id: &str) -> Result<Self, Box<dyn Error>> {
        let mut conn = get_connection_query()?;
        let row = conn.query_one(
            "UPDATE meetings 
