...
mod tests {
    ...
    /// Generate a self-contained ECDSA P-256 cert via `openssl`,
    /// configured exactly as `scripts/regen-dev-cert.sh` would, into
    /// `dir`. Returns `None` if openssl is missing on PATH so the test
    /// can be skipped in minimal CI environments.
    ///
    /// `validity_days` is the only knob that varies between callers; we
    /// reuse this helper to construct both passing fixtures (13 days,
    /// like the script) and the >14-day boundary case.
    fn generate_p256_cert(dir: &Path, validity_days: u32) -> Option<std::path::PathBuf> {
        let openssl = which_openssl()?;
        let key_path = dir.join("p256.key");
        let cert_path = dir.join("p256.pem");
        let cfg_path = dir.join("p256.cnf");
        ...
        assert!(key_status.success(), "openssl genpkey should succeed");
        ...
        let cert_status = Command::new(&openssl)
            ...
            .arg("-out")
            .arg(&cert_path)
            .args(["-days", &validity_days.to_string()])
            .arg("-config")
            .arg(&cfg_path)
            .args(["-extensions", "v3_req", "-sha256"])
            .status()
            .expect("run openssl req");
        assert!(cert_status.success(), "openssl req should succeed");
    ...
    fn p256_cert_with_correct_san_and_short_validity_passes() {
        // Self-contained: build a cert that satisfies every rule and
        // verify the validator accepts it. Locks the contract that the
        // regen script + WT preflight + wasm hash file all agree on.
        let dir = tempfile::tempdir().expect("tempdir");
        let cert_path = match generate_p256_cert(dir.path(), 13) {
            Some(p) => p,
            None => {
                eprintln!("skipping p256_cert_with_correct_san_and_short_validity_passes: openssl not on PATH");
                return;
            }
        };
        let der = load_pem_as_der(&cert_path);
        validate_chain(&[der], cert_path.to_str().unwrap())
            .expect("freshly minted P-256 13-day cert with SAN should pass preflight");
    }

    #[test]
    fn p256_cert_over_14_days_is_rejected() {
        // Boundary case for the 14-day cap. 30 days is well past the
        // limit; the validator must reject and name the validity issue.
        let dir = tempfile::tempdir().expect("tempdir");
        let cert_path = match generate_p256_cert(dir.path(), 30) {
            Some(p) => p,
            None => {
                eprintln!("skipping p256_cert_over_14_days_is_rejected: openssl not on PATH");
                return;
            }
        };
        let der = load_pem_as_der(&cert_path);
