...
//! Integration tests for the WebTransport server.
//!
//! These tests start the **real** WebTransport server (via `sec_api::webtransport::start`)
//! and exercise packet relay, lobby isolation, session lifecycle, and JWT-based authentication.
//!
//! Previously these lived inside `src/webtransport/mod.rs` as a `#[cfg(test)] mod tests` block.
...
//!
//! The packet-counting helpers (`increment_test_packet_counter_for_user`, etc.) remain
//! in the library crate at `sec_api::webtransport::test_helpers` because the production
//! code's `#[cfg(test)]` callback references them directly.
...
use sec_api::webtransport::test_helpers::{
    get_test_packet_counter_for_user, reset_test_packet_counters,
};
...
async fn connect_client(
    ...
    let base =
        std::env::var("WEBTRANSPORT_URL").unwrap_or_else(|_| "https://127.0.0.1:4433".to_string());
...
// =========================================================================
// Test: Packet relay between two WebTransport clients
// =========================================================================
                    ...
                    if !buf.is_empty() {
                        if let Ok(pkt) = VcPacketWrapper::parse_from_bytes(&buf) {
                            ...
                            if pkt.packet_type == media_type {
                                println!("Received MEDIA packet on B (size: {} bytes)", buf.len());
                                return Some(buf);
                            }
                            println!(
...
// =========================================================================
// Test: Lobby isolation (cross-lobby packets must not leak)
// =========================================================================
...
async fn test_lobby_isolation_impl() -> anyhow::Result<()> {
    ...
    for i in 1..=2 {
        let packet = create_test_packet(user_c, VcMediaType::VIDEO, format!("charlie-reply-{i}"));
    ...
    println!("After Bob's packet: A={alice_after_bob}, B={bob_after_bob}, C={charlie_after_bob}");
...
// =========================================================================
// Test: Meeting lifecycle (connect, disconnect, session end)
// =========================================================================
