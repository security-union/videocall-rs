...
async fn main() -> anyhow::Result<()> {
    ...
    let active_names: HashSet<&str> = active_participants.iter().map(|p| p.name.as_str()).collect();
    ...
    for (index, p) in active_participants.iter().enumerate() {
        ...
        let handle = tokio::spawn(async move {
            if let Err(e) = run_client(
                bot_config, user_id, meeting_id, audio_data, ekg_color, insecure, media_start, ld,
            )
            .await
            {
