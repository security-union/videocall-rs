...
pub fn HsvColorPicker(props: HsvColorPickerProps) -> Element {
    ...
    // Element size caches populated on pointerdown and cleared on pointerup.
    // `getBoundingClientRect()` forces a synchronous layout so calling it per
    // pointermove event (60Hz+) is wasteful; sizes are stable for the duration
    // of a drag under pointer capture, so we read them once and reuse.
    let mut sv_rect_cache = use_signal(|| None::<(f64, f64)>);
    let mut hue_rect_cache = use_signal(|| None::<(f64, f64)>);
    ...
    let on_sv_pointer_down = move |evt: PointerEvent| {
        if let Some(web_evt) = evt.try_as_web_event() {
            if let Some(el) = closest_element(&web_evt, ".color-picker-sv-square") {
                ...
                sv_rect_cache.set(Some(element_size(&el)));
            }
        }
    ...
    //
    // Pointer handlers attach to `.color-picker-hue-wrap--vertical` — not the
    // inner `.color-picker-hue-track` — so the wrap's horizontal padding
    // actually functions as an extended touch/hit target. Because the wrap
    // has no vertical padding and its single child (the track) stretches to
    // fill it (`display: flex; align-items: stretch`), the wrap's height
    // equals the track's height and `element_coordinates().y` on the wrap is
    // directly usable as the position along the hue gradient.
    let mut update_hue_from_event = move |evt: &PointerEvent| {
        let (_, h) = match hue_rect_cache() {
            Some(dims) => dims,
            None => {
                let web_evt = match evt.try_as_web_event() {
                    Some(e) => e,
                ...
                let Some(el) = closest_element(&web_evt, ".color-picker-hue-wrap--vertical") else {
                    return;
                };
                element_size(&el)
            }
    ...
    let on_hue_pointer_down = move |evt: PointerEvent| {
        if let Some(web_evt) = evt.try_as_web_event() {
            if let Some(el) = closest_element(&web_evt, ".color-picker-hue-wrap--vertical") {
                let _ = el.set_pointer_capture(web_evt.pointer_id());
                hue_rect_cache.set(Some(element_size(&el)));
            }
        }
    ...
    rsx! {
        div { class: "color-picker",
            div { class: "color-picker-top-row",
                ...
                div {
                    class: "color-picker-hue-wrap color-picker-hue-wrap--vertical",
                    // Pointer handlers on the wrap (not the track) so the
                    // horizontal padding is a usable hit area for touch and
                    // near-miss clicks. See update_hue_from_event for details.
