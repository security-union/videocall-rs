...
pub struct RuntimeConfig {
    ...
    /// can sustain. Video/screen only — audio's publish count is
    /// [`audio_published_layer_count`], independent of this flag.
    ///
...
/// Audio simulcast layers the microphone encoder publishes. NOT derived from
/// [`experimental_simulcast_max_layers`], which gates video/screen only. The
/// surviving layer is numbered 0 but runs at `AUDIO_QUALITY_TIERS[0]`, the TOP
/// bitrate, walked down by the AQ tier rather than by layer selection.
pub fn audio_published_layer_count() -> u32 {
    1
}
