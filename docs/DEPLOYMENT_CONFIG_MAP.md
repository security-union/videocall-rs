...
- **`WT_OUTBOUND_CHANNEL_CAPACITY`** (env var, read at startup) is resolved from `WT_OUTBOUND_CHANNEL_CAPACITY_DEFAULT` in `actix-api/src/constants.rs`, currently `512` (lowered from `4096` for fail-fast behaviour per issue #979 — a deep queue only buffers stale, useless video for slow receivers). The env override in helm is redundant but harmless; the code default is the source of truth. Raise it only for exceptional workloads.
