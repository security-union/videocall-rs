...
This API has hard requirements:

- ECDSA P-256 leaf cert
- Validity period ≤ 14 days
- SubjectAltName containing both `IP:127.0.0.1` and `DNS:localhost`
- Hash is SHA-256 of the DER-encoded cert (NOT the SPKI), base64-encoded
...
- `actix-api/certs/localhost.pem` (gitignored — short-lived)
- `actix-api/certs/localhost.key` (gitignored — short-lived)
