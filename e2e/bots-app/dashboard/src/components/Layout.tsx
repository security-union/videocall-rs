      ...
      <header className="border-b border-neutral-200 bg-white">
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-6">
          <div className="flex items-center gap-3">
            <Bot className="h-6 w-6 text-sky-500" aria-hidden="true" />
            <h1 className="text-lg font-semibold tracking-tight text-neutral-900">
              videocall <span className="text-neutral-500 font-normal">bots-app</span>
            </h1>
          </div>
          <div
          ...
          >
            <CircleDot
              className={`h-3 w-3 ${healthy ? "text-emerald-500" : "text-red-500"}`}
              aria-hidden="true"
            />
            <span className="text-neutral-600">
              {daemon ? (
                <>
                  ctl :{daemon.port}{" "}
                  <span className="font-mono text-xs text-neutral-400">pid {daemon.pid}</span>
                </>
              ) : (
