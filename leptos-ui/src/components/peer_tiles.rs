...
pub fn generate_for_peer_view(client: VideoCallClient, peer_id: String, full_bleed: bool) -> View<Box<dyn IntoView>> {
    ...
    if full_bleed && !is_screen_share_enabled_for_peer {
        let peer_video_div_id = Rc::new(format!("peer-video-{}-div", &peer_id));
        View::new_boxed(view! {
            <div class="grid-item full-bleed" id={(*peer_video_div_id).clone()}>
                <div class=move || if is_video_enabled_for_peer { "canvas-container video-on" } else { "canvas-container" }
                    on:click=move |_| { if is_mobile_viewport() { toggle_pinned_div(&peer_video_div_id); } }>
                    {move || if is_video_enabled_for_peer { View::new_boxed(view!{ <UserVideo id=peer_id.clone() hidden=false/> }.into_view()) } else { View::new_boxed(view!{ <div class=""><div class="placeholder-content"><span class="placeholder-text">{"Camera Off"}</span></div></div> }.into_view()) }}
                    <h4 class="floating-name" title=peer_id.clone() dir="auto">{peer_id.clone()}</h4>
                    <div class="audio-indicator">{if is_audio_enabled_for_peer { View::empty() } else { View::new_boxed(view!{<span>{"🔇"}</span>}) }}</div>
                    <button on:click=move |_| { toggle_pinned_div(&peer_video_div_id) } class="pin-icon">{"📌"}</button>
                </div>
            </div>
        })
    } else {
        let screen_share_div_id = Rc::new(format!("screen-share-{}-div", &peer_id));
        let peer_video_div_id = Rc::new(format!("peer-video-{}-div", &peer_id));
        View::new_boxed(view! {
            <>
            {move || if is_screen_share_enabled_for_peer { view!{
                <div class={move || if client.is_awaiting_peer_screen_frame(&peer_id) { "grid-item hidden" } else { "grid-item" }} id={(*screen_share_div_id).clone()}>
                    <div class="canvas-container video-on" on:click=move |_| { if is_mobile_viewport() { toggle_pinned_div(&screen_share_div_id); } }>
                        ...
                        <button on:click=move |_| { toggle_pinned_div(&screen_share_div_id) } class="pin-icon">{"📌"}</button>
                    </div>
                </div>
            ...
            <div class="grid-item" id={(*peer_video_div_id).clone()}>
                <div class=move || if is_video_enabled_for_peer { "canvas-container video-on" } else { "canvas-container" }
                    on:click=move |_| { if is_mobile_viewport() { toggle_pinned_div(&peer_video_div_id); } }>
                    {move || if is_video_enabled_for_peer { View::new_boxed(view!{ <UserVideo id=peer_id.clone() hidden=false/> }.into_view()) } else { View::new_boxed(view!{
...
fn UserVideo(id: String, hidden: bool) -> impl IntoView {
    ...
    leptos::prelude::Effect::new(move |_| {
        if let Some(canvas) = canvas_ref.get_untracked() {
            ...
            if let Some(ctx) = ctx { ctx.clear_rect(0.0, 0.0, canvas.width() as f64, canvas.height() as f64); }
        }
    });
...
fn toggle_pinned_div(div_id: &Rc<String>) {
