...
pub fn MeetingRoute() -> impl IntoView {
    ...
    let id = params.with_untracked(|p| p.get("id").map(|s| s.to_string()).unwrap_or_default());
    view! { <MeetingPage id/> }
}
...
pub fn MeetingPage(id: String) -> impl IntoView {
    ...
    // Create VideoCallClient storage (use StoredValue since VideoCallClient is not Send+Sync)
    let client = StoredValue::new(None::<VideoCallClient>);
    ...
    let request_permissions_fn = {
        ...
        move || {
            ...
            let on_denied = {
                let error_state = error_state;
                yew::Callback::from(move |e: JsValue| {
            ...
            access.on_granted = on_granted;
            access.on_denied = on_denied;
            access.request();
        }
    };
    ...
    let build_client = {
        ...
        move || {
            ...
            let opts = VideoCallClientOptions {
                ...
                on_connected: yew::Callback::from(move |_| {
                    log::info!("LEPTOS-UI: Connection established");
                }),
                on_connection_lost: yew::Callback::from(move |_| {
                    log::warn!("LEPTOS-UI: Connection lost");
                }),
                on_peer_added: yew::Callback::from(|email: String| {
                ...
                on_peer_first_frame: yew::Callback::from(|(_email, _media_type)| {}),
                on_peer_removed: None,
                get_peer_video_canvas_id: yew::Callback::from(|email| email),
                get_peer_screen_canvas_id: yew::Callback::from(|email| {
                    format!("screen-share-{email}")
                }),
    ...
    let request_permissions = move |_| {
        request_permissions_fn();
    };

    // Build meeting link signal
    let meeting_link = Memo::new(move |_| {
    ...
    let copy_meeting_link = Callback::new(move |_| {
            ...
            {move || if username_state.get().is_some() {
                ...
                view!{
                    <div id="grid-container" class={move || if force_desktop_grid_on_mobile() { "force-desktop-grid" } else { "" }}
                        data-peers={move || compute_num_peers_for_styling(client.get_value().as_ref())}
                        style={move || grid_container_style(client.get_value().as_ref())}>
                        {move || render_peer_tiles(client.get_value().as_ref())}

                        {move || if client.get_value().as_ref().map(|c| c.sorted_peer_keys().is_empty()).unwrap_or(true) {
                            view!{ <InviteOverlay meeting_link=meeting_link.get() on_copy=copy_meeting_link.clone() show_copy_toast=show_copy_toast.get() /> }.into_any()
                        } else { ().into_any() }}

                        {view!{ <HostControls
                            email={input_value_state.get()}
                            meeting_id=id_for_view.clone()
                            share_screen
                            mic_enabled
                            video_enabled
                            peer_list_open
                        ...
                        <div class={move || if client.get_value().as_ref().map(|c| c.is_connected()).unwrap_or(false) { "connection-led connected" } else { "connection-led connecting" }} title={move || if client.get_value().as_ref().map(|c| c.is_connected()).unwrap_or(false) { "Connected".to_string() } else { "Connecting".to_string() }}></div>
                    </div>
                }.into_any()
            } else {
                ...
                view!{
                    <div id="username-prompt" class="username-prompt-container">
                        <form on:submit=on_submit class="username-form">
                            ...
                            <input class="username-input" placeholder="Your name" pattern="^[a-zA-Z0-9_]*$" required autofocus on:keydown=on_keydown on:input=on_input value=input_value_state.get()/>
                ...
                view!{
                    <div id="join-meeting-container" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; background: #000000; z-index: 1000;">
                        <div style="text-align: center; color: white; margin-bottom: 2rem;">
                            ...
                            {move || error_state.get().map(|e| view!{ <p style="color: #ff6b6b; margin-top: 1rem;">{e}</p> }  )}
                        </div>
...
fn render_peer_tiles(client: Option<&VideoCallClient>) -> impl IntoView {
    ...
    view! {
        <div id="invite-overlay" class="card-apple" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 90%; max-width: 420px; z-index: 0; text-align: center;">
            ...
            <div style="display:flex; align-items:center; margin-top: 0.75rem; margin-bottom: 0.75rem;">
                ...
                <button class=move || if show_copy_toast { "btn-apple btn-primary btn-sm copy-button btn-pop-animate" } else { "btn-apple btn-primary btn-sm copy-button" } style="margin-left: 0.5rem;" on:click=move |_| on_copy.run(()) >
                    {"Copy"}
...
fn HostControls(
    ...
    client: StoredValue<Option<VideoCallClient>>,
) -> impl IntoView {
    // Toggle actions
                        ...
                        <nav class="video-controls-container">
                            <button class=move || if mic_enabled.get() { "video-control-button active" } else { "video-control-button" }
                                on:click=move |_| {
                                    ...
                                    if let Some(c) = client.get_value() { c.set_audio_enabled(mic_enabled.get()); }
                                }>
                            ...
                            <button class=move || if video_enabled.get() { "video-control-button active" } else { "video-control-button" }
                                on:click=move |_| {
                                    ...
                                    if let Some(c) = client.get_value() { c.set_video_enabled(video_enabled.get()); }
                                }>
                            ...
                            {move || if !is_ios() { view!{
                                <button class=move || if share_screen.get() { "video-control-button active" } else { "video-control-button" }
                                    on:click=move |_| {
                                        ...
                                        if let Some(c) = client.get_value() { c.set_screen_enabled(share_screen.get()); }
                                    }>
