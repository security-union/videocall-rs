        ...
        pub fn new(sample_rate: u32, channels: u8) -> Result<Self> {
            ...
            let error_cb = Closure::<dyn FnMut(JsValue)>::wrap(Box::new(move |e: JsValue| {
                ...
                web_sys::console::error_1(
                    &format!("[WebCodecs OpusDecoder] error: {}", msg).into(),
                );
            }));
...
mod stub_impl {
    ...
    /// Stub Opus decoder for non-web environments
    #[derive(Debug)]
    pub struct OpusDecoder {
        sample_rate: u32,
        channels: u8,
    ...
    impl SafariOpusDecoder {
        pub fn new(_sample_rate: u32, _channels: u8) -> Result<Self> {
            Err(NetEqError::DecoderError(
                "Safari decoder not available in non-web environment".to_string(),
            ))
        }
    }
