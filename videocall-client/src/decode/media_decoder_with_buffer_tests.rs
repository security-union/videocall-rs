...
mod tests {
    ...
    use crate::decode::media_decoder_with_buffer::{MediaDecoderWithBuffer, MIN_BUFFER_SIZE, MAX_BUFFER_SIZE};
    use std::sync::{Arc, Mutex};
    use videocall_types::protos::media_packet::{MediaPacket, AudioMetadata, VideoMetadata};
    use wasm_bindgen::prelude::*;
    use wasm_bindgen_test::wasm_bindgen_test;
    use web_sys::{CodecState, EncodedVideoChunkType, VideoDecoderInit, VideoDecoderConfig};
    ...
    impl MediaDecoderTrait for MockMediaDecoder {
        ...
        fn is_keyframe(&self, packet: &MediaPacket) -> bool {
            if self.use_audio {
                let chunk_type = EncodedAudioChunkType::from_js_value(
                    &JsValue::from(packet.frame_type.clone())
                ).unwrap();
    ...
    fn create_decoder(use_audio: bool) -> MediaDecoderWithBuffer<MockMediaDecoder> {
        ...
        let mut decoder: MediaDecoderWithBuffer<MockMediaDecoder> = MediaDecoderWithBuffer::new(&init).unwrap();
    ...
    fn test_video_basic_decode() {
        ...
        for i in 1..=MIN_BUFFER_SIZE {
            let result = decoder.decode(create_mock_video_packet(i as u64, EncodedVideoChunkType::Key, vec![i as u8]));
        ...
        for i in (MIN_BUFFER_SIZE as u64 + 1)..=(MIN_BUFFER_SIZE as u64 + 5) {
            decoder.decode(create_mock_video_packet(i, EncodedVideoChunkType::Delta, vec![i as u8]));
        }
    ...
    fn test_video_out_of_order_frames() {
        ...
        assert_eq!(
            decoder.decode(create_mock_video_packet(3, EncodedVideoChunkType::Delta, vec![3])),
            empty
        );
        
        assert_eq!(
            decoder.decode(create_mock_video_packet(1, EncodedVideoChunkType::Key, vec![1])),
            empty
        );
        
        assert_eq!(
            decoder.decode(create_mock_video_packet(5, EncodedVideoChunkType::Delta, vec![5])),
            empty
        );
        
        // Complete the buffer with more frames
        for i in 2..=10 {
            if i == 3 || i == 5 { continue; } // Skip already added sequences
            decoder.decode(create_mock_video_packet(i, EncodedVideoChunkType::Delta, vec![i as u8]));
        }
    ...
    fn test_video_keyframe_reset() {
        ...
        for i in 1..=(MIN_BUFFER_SIZE + 2) {
            decoder.decode(create_mock_video_packet(i as u64, EncodedVideoChunkType::Delta, vec![i as u8]));
        }
        
        // Insert a keyframe with a higher sequence number that should reset the buffer
        decoder.decode(create_mock_video_packet(20, EncodedVideoChunkType::Key, vec![20]));
    ...
    fn test_video_sequence_gap() {
        ...
        decoder.decode(create_mock_video_packet(large_sequence, EncodedVideoChunkType::Delta, vec![100]));
    ...
    fn test_video_buffering_logic_converges_fast() {
        ...
        for i in (MIN_BUFFER_SIZE as u64 + 1)..=(MIN_BUFFER_SIZE as u64 + 5) {
            let result = decoder.decode(create_mock_video_packet(i, EncodedVideoChunkType::Delta, vec![i as u8]));
    ...
    fn test_video_jitter() {
        ...
        for i in 1..=MIN_BUFFER_SIZE {
            decoder.decode(create_mock_video_packet(i as u64, EncodedVideoChunkType::Key, vec![i as u8]));
        }
    ...
    fn test_video_max_buffer_size() {
        ...
        decoder.decode(create_mock_video_packet(keyframe_seq, EncodedVideoChunkType::Key, vec![42]));
    ...
    fn test_audio_keyframe_reset() {
        ...
        // Filter for sequences from the first batch and second batch
        let first_batch = chunks.iter().filter(|c| c.audio_metadata.sequence < 20).count();
        let second_batch = chunks.iter().filter(|c| c.audio_metadata.sequence >= 20).count();
