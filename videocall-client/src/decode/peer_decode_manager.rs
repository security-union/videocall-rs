...
// Canvas provider - only needed for non-yew-compat mode
#[cfg(not(feature = "yew-compat"))]
use crate::canvas_provider::CanvasIdProvider;
...
/// PeerDecodeManager struct with Yew callbacks (yew-compat feature)
#[cfg(feature = "yew-compat")]
#[derive(Debug)]
...
/// PeerDecodeManager struct with CanvasIdProvider (framework-agnostic)
#[cfg(not(feature = "yew-compat"))]
#[derive(Debug)]
pub struct PeerDecodeManager {
    connected_peers: HashMapWithOrderedKeys<String, Peer>,
    canvas_id_provider: Rc<dyn CanvasIdProvider>,
    diagnostics: Option<Rc<DiagnosticManager>>,
}

// Default implementation for yew-compat
#[cfg(feature = "yew-compat")]
...
// Framework-agnostic constructors
#[cfg(not(feature = "yew-compat"))]
impl PeerDecodeManager {
    pub fn new_with_canvas_provider(canvas_id_provider: Rc<dyn CanvasIdProvider>) -> Self {
        Self {
            connected_peers: HashMapWithOrderedKeys::new(),
            canvas_id_provider,
            diagnostics: None,
    ...
    pub fn new_with_canvas_provider_and_diagnostics(
        canvas_id_provider: Rc<dyn CanvasIdProvider>,
        diagnostics: Rc<DiagnosticManager>,
    ) -> Self {
        Self {
            connected_peers: HashMapWithOrderedKeys::new(),
            canvas_id_provider,
...
// Common methods that work for both yew-compat and framework-agnostic modes
impl PeerDecodeManager {
    ...
    pub fn run_peer_monitor(&mut self) {
        ...
        for k in removed {
            // Emit to event bus
            emit_client_event(ClientEvent::PeerRemoved(k));
        }
    }
    ...
    fn add_peer(&mut self, email: &str, aes: Option<Aes128State>) -> Result<(), JsValue> {
        ...
        self.connected_peers.insert(
            email.to_owned(),
            Peer::new(
                self.canvas_id_provider.get_video_canvas_id(email),
                self.canvas_id_provider.get_screen_canvas_id(email),
                email.to_owned(),
                aes,
            )?,
        );
