...
/// Shared health reporting loop used by both yew and non-yew modes.
/// The `send_fn` closure abstracts the difference between `Callback::emit` and direct function call.
#[allow(clippy::too_many_arguments)]
fn run_health_reporting_loop(
    peer_health_data: Rc<RefCell<HashMap<String, PeerHealthData>>>,
    session_id: String,
    meeting_id: String,
    reporting_peer: String,
    send_fn: Rc<dyn Fn(PacketWrapper)>,
    interval_ms: u64,
    reporting_audio_enabled: Rc<RefCell<bool>>,
    reporting_video_enabled: Rc<RefCell<bool>>,
    active_server_url: Rc<RefCell<Option<String>>>,
    ...
    let peer_health_data = Rc::downgrade(&peer_health_data);
    let audio_enabled = Rc::downgrade(&reporting_audio_enabled);
    let video_enabled = Rc::downgrade(&reporting_video_enabled);
    let active_server_url = Rc::downgrade(&active_server_url);
    let active_server_type = Rc::downgrade(&active_server_type);
    let active_server_rtt_ms = Rc::downgrade(&active_server_rtt_ms);
        ...
        loop {
            ...
            if let Some(peer_health_data) = Weak::upgrade(&peer_health_data) {
                if let Ok(health_map) = peer_health_data.try_borrow() {
                    ...
                    let health_packet = HealthReporter::create_health_packet(
                        &session_id,
                        &meeting_id,
                        &reporting_peer,
                        &health_map,
