//! Transport-layer network-condition simulator.
//!
//! A [`NetSimShim`] shapes the byte stream flowing through a single
//! direction (uplink or downlink) between the caller's application layer
//! and its transport (WebTransport uni streams, WebTransport datagrams,
//! or WebSocket frames).
//!
//! Because the shim operates on the application byte stream, every
//! transport sees it identically: the QUIC / TLS handshake is below the
//! shim and is never impaired.
//!
//! Model:
...
//! - Bernoulli duplicate using `duplicate_pct` — caller is responsible
//!   for emitting the extra copy.
//! - `reorder_pct`: a coarse approximation. Each admission is
//!   independent, and because per-packet delays are sampled
//!   independently, jittered packets already finish in a shuffled
//!   order. When `reorder_pct > 0` we additionally draw a second
//!   small-but-noticeable delay on a fraction of packets to amplify
...
/// `[0, 2 * jitter_ms]` on top of `latency_ms`. That is, the total
/// one-way delay falls in `[latency_ms, latency_ms + 2 * jitter_ms]`,
/// with mean `latency_ms + jitter_ms`. This is the single source of
/// truth for the jitter distribution used throughout the shim and its
/// tests.
#[derive(Debug, Clone, PartialEq)]
pub struct NetworkProfile {
    /// Base one-way latency added to every admitted packet.
    pub latency_ms: u32,
    /// Jitter magnitude. Adds a uniform `[0, 2 * jitter_ms]` extra
    /// delay. See the struct-level doc for the full convention.
    pub jitter_ms: u32,
    ...
    /// extra random delay applied to a fraction of packets — see module
    /// docs.
    pub reorder_pct: f32,
    /// Outbound bandwidth cap in kilobits/second. `None` means
    /// unlimited.
    pub uplink_kbps: Option<u32>,
    /// Inbound bandwidth cap in kilobits/second. `None` means
    /// unlimited.
    pub downlink_kbps: Option<u32>,
    /// Deterministic RNG seed (for tests). `None` draws an OS-random
    /// seed.
    pub seed: Option<u64>,
}
...
impl NetworkProfile {
    /// A profile that applies no impairment. `is_passthrough` returns
    /// `true`.
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        // Cap jitter at the same ceiling as latency. The admit path
        // uses `2 * jitter_ms` and `4 * jitter_ms`, so this also serves
        // as defense in depth against integer overflow (see the
        // `saturating_mul` calls in `admit`). Unreachable via the
        // normal YAML/CLI path today because no preset sets this above
        // a few hundred ms, but test-only callers can construct a
        // profile directly so we check here too.
...
/// `uplink_kbps` / `downlink_kbps` entry is honored for bandwidth
/// shaping.
...
/// Result of a single [`NetSimShim::admit`] call. The caller applies
/// the decision: sleep for the given duration, emit a duplicate, or
/// drop.
#[derive(Debug, Clone, PartialEq)]
pub enum Admission {
    /// Forward immediately.
...
impl NetSimShim {
    /// Create a shim for the given profile and direction. The
    /// per-direction rate limit is applied iff the corresponding
    /// `*_kbps` field is set.
    ...
    pub fn admit(&self, size_bytes: usize) -> Admission {
        ...
        //
        // Uniform jitter in [0, 2*jitter_ms]. See `NetworkProfile`
        // struct doc. `saturating_mul` guards against overflow if a
        // future caller skips `validate()` — jitter is capped at 5000ms
        // there, but keeping the hot path safe by construction is
        ...
        //
        // Reorder is already implicit via independent per-packet
        // jitter — this extra term bumps out-of-order probability when
        // users ask for noticeable reorder specifically.
        if self.profile.reorder_pct > 0.0 {
            let roll: f32 = rng.gen::<f32>() * 100.0;
            if roll < self.profile.reorder_pct {
                // Draw a second uniform delay from [0, 4*jitter_ms] or
                // a fixed 20ms floor when jitter_ms is 0.
                // `saturating_mul` mirrors the guard above on the
                // jitter term.
...
impl TokenBucket {
    /// Attempt to take `size_bytes` from the bucket. Returns the
    /// additional delay the caller must wait before the packet can be
    /// considered sent.
    ///
    /// When the bucket is short, we model the caller sleeping for the
    /// returned duration: the bucket's tokens are drained to zero AND
    /// `last_refill` is advanced forward by the wait. That way,
    /// back-to-back admits that each find the bucket empty correctly
    /// accrue successively larger delays rather than each one starting
    /// from the same "bucket full in dt" baseline.
    fn consume(&mut self, size_bytes: usize) -> Duration {
        ...
        // Drain the bucket and pretend `wait_secs` of wall clock has
        // already passed — the caller is contracted to sleep that long.
        self.tokens = 0.0;
...
mod tests {
    ...
    fn jitter_range() {
        ...
        // With 1000 samples across a 100ms range we expect to see near
        // the extremes.
    ...
    fn token_bucket_rate_limit() {
        ...
        // 10 × 50KB = 500KB total. After the 125KB burst, the
        // remaining 375KB must be drained at 125KB/s → ~3 seconds of
        // accumulated delay.
        let profile = NetworkProfile {
        ...
        // Allow 1ms of floating-point slack; the exact theoretical
        // total is 0.2 + 0.4 × 7 = 3.0s but accumulated secs_f64 math
        // can underflow.
        assert!(
    ...
    fn bucket_cap_prevents_starvation() {
        // An oversized packet gets clamped at 5 seconds so the
        // pipeline doesn't wedge behind it.
        let profile = NetworkProfile {
            uplink_kbps: Some(8),
            seed: Some(0),
            ..Default::default()
    ...
    fn admit_saturating_mul_survives_large_jitter() {
        // Construct a profile that bypasses `validate()` with a jitter
        // value large enough that naive `2 * jitter_ms` /
        // `4 * jitter_ms` would overflow u32. `admit()` must still
        // return a sane Admission.
    ...
    fn direction_selects_rate_limit() {
        ...
        // The Down shim should Pass because neither latency nor a
        // matching rate limit is configured.
