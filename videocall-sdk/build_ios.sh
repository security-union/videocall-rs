...
codesign -s - target/aarch64-apple-ios/release/libvideocall_uniffi.a
codesign -s - target/aarch64-apple-ios-sim/release/libvideocall_uniffi.a
codesign -s - target/aarch64-apple-darwin/release/libvideocall_uniffi.a

# Create XCFramework
echo "Creating XCFramework..."
rm -rf target/VideoCallIOS.xcframework
xcodebuild -create-xcframework \
  -library target/aarch64-apple-ios/release/libvideocall_uniffi.a \
  -headers target/swift/include \
    -library target/aarch64-apple-ios-sim/release/libvideocall_uniffi.a \
    -headers target/swift/include \
  -library target/aarch64-apple-darwin/release/libvideocall_uniffi.a \
  -headers target/swift/include \
...
# Update Package.swift to use the correct path
echo "Updating Package.swift to use the correct path..."
sed -i '' 's|path: "../../target/VideoCallIOS.xcframework"|path: "Frameworks/VideoCallIOS.xcframework"|g' "$SCRIPT_DIR/VideoCallKit/Package.swift"

# Clean the build directory to avoid module cache issues
echo "Cleaning build directory..."
rm -rf "$SCRIPT_DIR/VideoCallKit/.build"
