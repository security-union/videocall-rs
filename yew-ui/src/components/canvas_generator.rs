...
/// the video tile will occupy the full grid area. If `host_id` matches `key`, a crown badge
/// is displayed to indicate the meeting host.
pub fn generate_for_peer(
    ...
    let is_host = host_id.map(|h| h == key).unwrap_or(false);
        ...
        return html! {
            <div class="grid-item full-bleed" id={(*peer_video_div_id).clone()}>
                ...
                >
                    ...
                    <h4 class="floating-name" title={key.clone()} dir={"auto"}>
                        {if is_host { html!{ <span class="host-badge"><CrownIcon/></span> } } else { html!{} }}
                        {key.clone()}
                    </h4>
