...
use crate::constants::{app_config, login_url};
...
async fn fetch_auth_config(api_base_url: &str) -> Option<AuthConfig> {
    let url = format!("{}/auth/config", api_base_url.trim_end_matches('/'));
    let resp = reqwasm::http::Request::get(&url).send().await.ok()?;
    resp.json::<AuthConfig>().await.ok()
}
...
pub fn login() -> Html {
...

    {
        ...
        use_effect_with((), move |_| {
            spawn_local(async move {
                if let Ok(cfg) = app_config() {
                    if let Some(auth) = fetch_auth_config(&cfg.api_base_url).await {
    ...
    let is_google = provider.eq_ignore_ascii_case("google");

    let button = if is_google {
        html! {
            <button
                onclick={login}
                class="transition-transform hover:scale-[1.02] active:scale-[0.98]"
                style="background: #ffffff; border: 1px solid #747775; padding: 10px 24px; cursor: pointer; border-radius: 4px; display: flex; align-items: center; gap: 12px; font-family: 'Google Sans', Roboto, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; font-size: 14px; font-weight: 500; color: #1f1f1f; letter-spacing: 0.25px;"
            >
                // Google "G" logo SVG per branding guidelines
