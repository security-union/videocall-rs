...
use crate::modals::create_meeting_modal::CreateMeetingModal;
use crate::context::{
...
pub fn home() -> Html {
    ...
    let handle_create_meeting = {
        let username_ref = username_ref.clone();
        let navigator = navigator.clone();
        let username_ctx = username_ctx.clone();
        Callback::from(move |password: Option<String>| {
            let username = username_ref.cast::<HtmlInputElement>().unwrap().value();
            ...
            if let Some(pwd) = password {
                web_sys::console::log_1(&format!("Meeting password: {}", pwd).into());
            }
    ...
    html! {
        <>
            <CreateMeetingModal
                ...
                on_create={handle_create_meeting}
            />
